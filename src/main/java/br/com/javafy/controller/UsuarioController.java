...
public class UsuarioController {
    ...
    public ResponseEntity<UsuarioDTO> update (@PathVariable("idUser") Integer id
                                                , @Valid @RequestBody UsuarioCreateDTO usuario) throws PessoaNaoCadastradaException, BancoDeDadosException {
        return ResponseEntity.ok(usuarioService.update(usuario,id));
    }

    @DeleteMapping("/{idUser}")
    public void delete (@PathVariable ("idUser") Integer id) throws PessoaNaoCadastradaException, BancoDeDadosException {
        usuarioService.delete(id);
    }

    @GetMapping("/byname")
    public  ResponseEntity<List<UsuarioDTO>> listByName (@RequestParam("nome=") String nome) throws BancoDeDadosException {
